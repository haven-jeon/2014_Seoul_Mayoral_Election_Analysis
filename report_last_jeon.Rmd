...
```{r eda, message=FALSE, warning=FALSE,dev='png',dpi=600, fig.width=12}
...
```{r mc,message=FALSE, warning=T, dev='png',dpi=600, fig.width=12}
...
## Comparison between Frequentist[^1] and Bayesian 

```{r comp, ,message=FALSE, warning=T, dev='png',dpi=600,fig.width=12}
...
ci_r <- qnorm(0.975) * 1/sqrt(sumtotal) * 
  sqrt(p_p *(1- p_p) + p_j * (1 - p_j) + 2 * p_p * p_j)
ymax <- freq_diff_r + ci_r
ymin <- freq_diff_r - ci_r

res <- rbind(res, data.frame(dt=ymd('2014-06-01'), diff_kind="freq_total", 
                             diff_ratio=freq_diff_r, ci=ci_r))
...
ggplot(res, aes(x=dt,y=diff_ratio, colour=diff_kind)) + geom_point() + geom_line() + 
  ...
  geom_text(data=NULL,x=as.numeric(ymd('2014-04-01')) , y=freq_diff_r, 
            label="6.4 Result", vjust=0, colour='black', size=5) + 
  scale_x_datetime("date",breaks = "7 days") +
  scale_y_continuous( expression(theta[p] - theta[j]), breaks=seq(-0.01,0.25, by=0.01)) + 
  scale_color_discrete("Inference Methods")

```

[^1]: $\hat{p_p} - \hat{p_j} \pm \frac{Z_{a/2}}{\sqrt{N}}\sqrt{\frac{N - n}{N}}\sqrt{\hat{p_p}(1 - \hat{p_p}) + \hat{p_j}(1 - \hat{p_j}) + 2\hat{p_p}\hat{p_j}}$
